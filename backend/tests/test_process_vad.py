...
def test_process_vad_durations():
    ...
    assert (
        len(chunks) >= 2
    ), f"Audio was {duration_s}s, should be split to respect max 15s duration"

    for chunk in chunks:
        assert (
            5.0 <= chunk["duration"] <= 15.0
        ), f"Chunk duration {chunk['duration']} out of bounds"
...
def test_process_vad_strips_silence():
    ...
    assert (
        chunk["start_time"] >= 1.5
    ), f"Expected silence to be stripped, but start time is {chunk['start_time']}"
    ...
    assert (
        chunk["duration"] >= 5.0
    ), f"Chunk duration {chunk['duration']} should be padded to 5s"
