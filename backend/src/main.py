...
def main():
    ...
    print(f"[AgenticSpeech] Starting pipeline with BATCH_SIZE={batch_size} and MAX_WORKERS={max_workers}")
    ...
    end_time = time.time()
    print(f"[AgenticSpeech] Pipeline finished. Processed {processed_count} chunks in {end_time - start_time:.2f} seconds.")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                if not pass_status:
                    print(f"  [Gate] Dropped chunk due to high WER: {final_state.get('wer_score')}")
